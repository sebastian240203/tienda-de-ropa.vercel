...
function Navbar() {
  ...
  const { vaciarCarrito, carrito, setMostrarModalCarrito } = useCartContext();
  ...
  // Función para cerrar el menú (usando clases para compatibilidad)
  const cerrarMenu = () => {
  ...
  // Agregado: Cerrar menú automáticamente en cualquier cambio de ruta
  useEffect(() => {
    const handleRouteChange = () => {
      cerrarMenu();
    };

    // Listener para cambios de historia (navegación)
    window.addEventListener('popstate', handleRouteChange);

    // Cerrar menú en cambio de pathname
    handleRouteChange();
    ...
    <>
      <NavbarContainer className="navbar navbar-expand-lg navbar-dark fixed-top">
        <div className="container-fluid">
          <Logo to="/" className="navbar-brand">Tienda </Logo>
            ...
            <SeccionUsuario className="d-flex align-items-center gap-3">
              <ContenedorCarrito> 
                <IconoCarrito onClick={() => {
                  console.log("Clic en ícono del carrito");
                  setMostrarModalCarrito(true);
                  cerrarMenu();  // Cerrar menú al abrir carrito
...
const IconoCarrito = styled.span`
  color: white !important;
  text-decoration: none;
  padding: 0.5rem;
  display: flex;
